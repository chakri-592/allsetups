...
sed -i '58  a\<user username="tomcat" password="raham123" roles="manager-gui, manager-script"/>' apache-tomcat-9.0.96/conf/tomcat-users.xml
...
#in order to update the tomcat version in line-2 got dlcdn.apache.org/tomcat--->tomcat-9--->where you can find the version.
